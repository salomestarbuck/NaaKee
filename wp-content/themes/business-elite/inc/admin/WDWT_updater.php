...
class WDWT_updater{
              ...
              if($this->compare_version($version, $this->version_old)){
                foreach($changes as $change){
                  ...
                  if(isset($change['type'])){
                    ...
                    $old_meta[$new_name] = $this->$function_name($old_meta[$old_name], $args); 
                  }
                  else{
                    /*if only name changed*/
                    $old_meta[$new_name] = $old_meta[$old_name];  
                  }
                  //var_dump($old_meta[$new_name]);
                  ...
                  elseif(isset($change['external']) && $change['external'] == true ){
                    ...
                    if(!empty($old_meta)){
                      if(isset($change['type'])){
                        ...
                        $function_name = $change['type'];
                        $args=$change['args'];
                        if(!isset($change['save_external']) ){
                          $old_meta[$new_name] = $this->$function_name($old_val, $args); 
                        }
                        else{
                        
                          $new_val = $this->$function_name($old_val, $args); 
                          if(! update_post_meta( $current_post ->ID, $old_name, $new_val)){
                            add_post_meta( $current_post ->ID, $old_name, $new_val);
                          }
                          //var_dump($new_val);
                        }                        
                      }
  ...
  */
   ...
   * get posts and categories createdwith checkboxes
  */
  protected function get_old_posts_cats_meta($val, $args=array()){
    $value = $val;
    if(is_integer($value)){
      $result = array($value);
      return $result;
  ...
  /*---------*/
     /*---------*/
  protected function _wp_page_template($val,$args=array()){
	if(is_null($val)){
		return null;
	} 
